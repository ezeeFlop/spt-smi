...
def send_messages(socket):
    for i in range(10):
        message = f"Message de Process 2 - {i}"
        print(f"Process 2 envoi : {message}")
...
def receive_messages(socket):
    while True:
        try:
            ...
            print(f"Process 2 reçu : {message}")
...
def main():
    ...
    send_socket.bind("tcp://127.0.0.1:5558")

    # Socket pour recevoir des messages
    receive_socket = context.socket(zmq.PULL)
    receive_socket.connect("tcp://127.0.0.1:5557")
